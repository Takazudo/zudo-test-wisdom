...
description: ASTプラグインを2つの相補的なレイヤーでテストする -- ユニット制御のための手組み mdast ツリーファクトリと、UPDATE_FIXTURES による再生成と言語をまたぐパリティオラクルを備えた、パイプライン全体の一致を保証するゴールデンフィクスチャ群。
sidebar_position: 3
---
...
バイト単位という基準は**忠実な移植**に固有のものです -- 同じパイプラインを別言語で再実装したものであり、参照が構造的に真実を定義するため、あらゆる差異は移植のバグになります。この基準は、あるライブラリを別のライブラリへ差し替える場合には引き継がれません。差し替え先のライブラリは自分たちのデータスナップショットと意図的に修正したバグを抱えているため、そこでは小さな*分類済み*の差分が正しい結末であることが少なくありませんし、バイト単位の同一性を要求することは、より正しいという理由で候補を却下することを意味します。[依存ライブラリ差し替えのテスト](./dependency-replacement-testing.mdx)は、このパターンの規律 -- 固定された参照と機械的な差分 -- をそのまま引き継ぎつつ、受け入れ基準だけをそのケースに必要なものに置き換えます。差異ゼロではなく、*未分類の*差異ゼロです。
