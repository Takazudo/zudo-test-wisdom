...
`cd core && cargo test` がプロジェクト唯一のRustゲートになると、Tauri依存に触れるためアプリクレートへ追加された `#[cfg(test)]` モジュールは、CIでもpre-pushゲートでも定期再試験でも実行されないまま、黙って取り残されます。実際に、リダイレクト時のフェイルクローズ、SSE境界のパース、トークンの秘匿化を扱うセキュリティ上重要な7件のトランスポートテストが、どの自動化レベルにも含まれないアプリクレートに置かれていました（zudolab/zudo-text#5855）。
