...
これは重いローカルゲートだけに制限し、どこにでも適用しないでください。開発中の素早いフィードバックのため、対話的な `pnpm test:unit` は無制限のままにし、CI ランナーには手を加えないでください -- 通常すでにコア数が少なく自己制限的なので、そこに明示的な上限を追加しても不要な天井を増やすだけです。
