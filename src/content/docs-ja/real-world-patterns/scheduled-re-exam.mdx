...
while IFS=$'\t' read -r FILE TITLE; do
  echo "::warning file=${FILE}::${TITLE} -- passed only after retry, record and schedule the fix"
  ...
  # Key the tracking issue on file + title. Keyed on title alone, two specs
  # sharing a name collect telemetry into one issue that neither fix closes.
  FLAKY_TITLE="flaky: ${FILE} › ${TITLE}"
...
while IFS=$'\t' read -r STATUS FILE TITLE; do
  ...
  # Scope to $FILE, not all of e2e/ -- head -1 across the tree picks the wrong
  # issue whenever two specs share a title.
  ISSUE_URL="$(grep -n -B1 -F "\"${TITLE}" "$FILE" 2>/dev/null \
