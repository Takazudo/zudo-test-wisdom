...
while IFS=$'\t' read -r STATUS TITLE; do
  ...
  # Recursive `grep -rn --include` matches scripts/guard-flaky-paper-trail.sh's discovery
  # convention -- bash's e2e/**/*.spec.ts glob silently drops specs at the wrong nesting
  # depth without `shopt -s globstar`
  ISSUE_URL="$(grep -rn --include='*.spec.ts' -B1 -F "\"${TITLE}" e2e/ 2>/dev/null \
