    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 22
          cache: pnpm
      # No --frozen-lockfile: resolve against the live registry, not the pin
      - run: pnpm install
...
**ランナーの癖：JSONレポーターのリダイレクト。** Playwright 1.58は、古い例でまだ見かける `--reporter=json:<path>` のコロン構文を拒否します -- レポートを書き出す代わりにエラーになります。上のexamワークフローのスケルトンですでに使われている `PLAYWRIGHT_JSON_OUTPUT_NAME` が、現行Playwrightでサポートされている、JSONレポーターの出力パスのリダイレクト方法です。
...
**トレースはHTMLレポートのディレクトリではなく、ランナーの `outputDir` に置かれます。** Playwrightのデフォルトの `outputDir` は `test-results/` です -- 失敗・リトライした試行のトレース、スクリーンショット、動画はそこに書き出されます。HTMLレポート（上のexamワークフローで使われている `playwright-report/`）は結果をまとめる別のディレクトリであり、生のトレースファイルを自動的にバンドルしません。`playwright-report/` だけを取得するアーティファクトアップロードのステップは、トレースを静かに全部失います：
