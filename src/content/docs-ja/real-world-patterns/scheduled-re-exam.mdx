...
# The tree's ignored set, as fully-qualified names. A raw grep of `#[ignore = "`
# finds the attribute but not the module path; nextest resolves both.
tree_names() {
  cargo nextest list --run-ignored ignored-only --message-format json \
