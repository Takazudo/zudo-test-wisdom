...
修正策は、シングルソース・オブ・トゥルースのメタテストパターンと同様です。ローカルのプッシュ前ゲートとCIの両方に組み込む、小さなスクリプトで `e2e/*.spec.ts` のすべてのファイル名が既知のプロジェクトプレフィックスで始まることをアサートします：

```bash
# Every e2e spec must start with a known project prefix, or it runs nowhere.
known='fixtureA|fixtureB|fixtureC'
bad=$(ls e2e/*.spec.ts | grep -Ev "/($known)[^/]*\.spec\.ts$" || true)
...
<Tip>
`waitForLoadState("networkidle")` は、リクエストを発火しないSPAナビゲーションで使われる典型的なアンチパターンです — ビューが準備できる前に解決することがあります。実際の完了シグナルとして、ウェブファーストのアサーション（`expect(...).toBeVisible()` など）やイベントキーの待機を使用してください。詳しいカタログとレシピは [フレイク根本原因カタログ & デフレイキングレシピ](./deflaking-recipe.mdx) を参照してください。
</Tip>
