...
# Nothing to check if the baseline hasn't been introduced yet.
[ -f "$BASELINE_FILE" ] || exit 0

failed=0

while IFS=" " read -r expected_count path; do
  # Count waitForTimeout calls that lack a // wait-ok: comment in the 2 lines above.
  actual=$(grep -n "waitForTimeout" "$path" 2>/dev/null | while IFS=":" read -r lineno _rest; do
    # Check the two preceding lines for the wait-ok marker.
    start=$(( lineno - 2 ))
    [ "$start" -lt 1 ] && start=1
    if ! sed -n "${start},$((lineno - 1))p" "$path" | grep -q "wait-ok:"; then
      echo "found"
    fi
  done | wc -l | tr -d ' ')

  if [ "$actual" -gt "$expected_count" ]; then
    echo "FAIL $path: $actual unannotated waits (baseline $expected_count) — annotate new waits with // wait-ok: <why>"
    failed=1
  elif [ "$actual" -lt "$expected_count" ]; then
    echo "FAIL $path: baseline is stale ($expected_count → $actual) — shrink the baseline to $actual"
    failed=1
  fi
done < "$BASELINE_FILE"
