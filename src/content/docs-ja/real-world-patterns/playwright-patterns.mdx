...
上の「本番ビルドでテストせよ」というアドバイスには、モノレポ特有の盲点があります。多くのアプリスイートは本番ビルドに対して*実行できない*のです：MSWのサービスワーカーセットアップ、dev限定ルート、dev限定のテストブリッジ（`import.meta.env.DEV` ガードの内側で `window` に公開されるstate）は、いずれも開発サーバーの下にしか存在しません。しかし、pnpmワークスペースのモノレポで開発サーバーを走らせることには、隠れたテストごとの税金があります。
...
- **`existsSync` ガードはthrowする。** これがないと、`dist` が欠けていたり古かったりしたときに静かにソース配信へフォールバックし（さらに悪ければ古いバンドルをテストし）、このレーンの数値が無意味になります。
- **フラグはオプトイン。** ローカルの `pnpm test:e2e` とインタラクティブな開発はソース配信とHMRを維持し、CI（およびCIを再現したい人）だけがグラフ圧縮のための約15秒のビルドを払います。
- **バンドルするのは重くて安定したパッケージだけ。** アプリ自身のソース — テスト対象のコード — は引き続きライブで配信されます。バンドルしているのは依存であって、被検体ではありません。
