...
両者を区別することが重要なのは、デフレーキングの定石である「壁時計待機をイベント起点の待機に置き換える」が、後者にはまったく効かないからです。置き換えるべき壁時計待機はもう残っておらず、正当化すべき数値だけが残っています。2つの意味が共有しているのは、たった1つの規律です: どちらを引き上げるにも、計測されたレイテンシ分布を根拠にすること、決して勘で決めないこと。上の60秒というプロジェクト予算がこれを満たしているのは、それが収容すべき最大の子プロセスタイムアウトから導かれているからです。赤い実行を緑に変えるために選ばれた数値は、フレーキーを取り除くのではなく隠すだけです。
