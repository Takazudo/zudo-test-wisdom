...
**「ガードレール」はこのガイド内で2つの異なる意味で使われています。**このページでは*ランナーレベル*のもの、つまり本当にハングしたテストを速やかに失敗させるための、厳格なデフォルトの `testTimeout` を指します。一方、デフレーキングレシピの[「すでにイベントに同期している待機の、ハング検知用の固定期限」の項目](../test-integrity/deflaking-recipe.mdx#part-1-—-根本原因カタログ)（Part 1 の根本原因カタログにあるネイティブスイート向けの注記）では、*待機ごと*のものを指します。これは**すでに**イベントに同期している待機に設けた固定期限で、同期の取り方は正しく、残る論点は期限の値に根拠があるかどうかだけです。

両者を区別する必要があるのは、デフレーキングの定石である「一定時間の待機をイベントに同期する待機へ置き換える」が、後者には効かないからです。置き換えるべき待機はもうなく、根拠を示すべき数値だけが残っています。どちらの値を引き上げる場合も、**通常時と負荷時のレイテンシ分布の実測、または予算内に収める必要のある子プロセスの期限のいずれかから**導いたことを示してください。勘で決めてはいけません。上の60秒というプロジェクト予算は、収める必要のある最大の子プロセスタイムアウトから計算されているため、この根拠の要件を満たしています。失敗した実行を通すためだけに選んだ数値は、フレイクを隠すだけです。[ルール8 — ゲートを不正操作しない](../decision-guide/required-behavior.mdx#4つの実行ルール-ルール8-—-ゲートを不正操作しない)に従い、オープンなissueへのリンクも付け、ゲートを定義するランナー設定への編集として、他の変更と同じフレッシュコンテキストのレビューを受けてください。
...
[mdx-formatter](https://github.com/Takazudo/mdx-formatter)（Rust（napi）エンジン上のNode CLI + npmライブラリであるMarkdown/MDXフォーマッター）のパターン：ユニットテスト、APIテスト、関数テスト用の別々の設定：
...
pgen（Cloudflare Workersバックエンドを持つ非公開のGPUパターンジェネレーター）の実例：MiniflareによるローカルD1データベースとR2ストレージを使ったCloudflare Workersのテスト：
...
happy-dom は高速ですが、自分のコードをいくら読み返しても気づけない挙動がいくつかあります。これらはフレームワーク固有のクセだからです。zfb-runtime -- このサイトが走るビルドツールである[zfb](https://github.com/Takazudo/zudo-front-builder)のパッケージ -- のクライアントルーターのテストから、特に発見に時間を要したエスケープハッチと、import 順序に関する罠を紹介します。
