...
**「ガードレール」はこのガイド内で2つの異なる意味で使われています。**このページでのそれは*ランナーレベル*のもの、つまり本当にハングしたテストを高速な失敗に変えることだけを仕事にしている、厳格なデフォルトの `testTimeout` です。一方、[デフレーキングレシピ](../test-integrity/deflaking-recipe.mdx)（Part 1 の原因カタログにあるネイティブスイート向けの注記）でのそれは*待機ごと*のもの、つまり**すでに**イベントを起点にしている待機の上に重ねられた固定の締め切りです。同期の取り方はすでに正しく、残る論点はその締め切りの値に根拠があるかどうかだけです。

両者を区別することが重要なのは、デフレーキングの定石である「壁時計待機をイベント起点の待機に置き換える」が、後者にはまったく効かないからです。置き換えるべき壁時計待機はもう残っておらず、正当化すべき数値だけが残っています。2つの意味が共有しているのは、たった1つの規律です: どちらを引き上げるにも、示せる導出 -- 計測されたレイテンシ分布か、その予算が収容すべき子プロセスの締め切り -- を根拠にすること、決して勘で決めないこと。上の60秒というプロジェクト予算がこれを満たしているのは、それが収容すべき最大の子プロセスタイムアウトから導かれているからです。赤い実行を緑に変えるために選ばれた数値は、フレーキーを取り除くのではなく隠すだけです。
