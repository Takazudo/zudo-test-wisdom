...
プレーンな `cargo test` では、この予算は実装不可能です — 組み込みのハーネスにはリトライの仕組みがまったくありません。Rustスイートが `retries`・pass-on-retryのテレメトリ・テストごとのタイムアウトをどう得るかは、[nextestへの移行レシピ](../real-world-patterns/cargo-nextest-migration.mdx)を参照してください。
...
- [CIランナーのサイジング](../tools-reference/ci-runner-sizing.mdx) -- このページが前提とするT2/T3のハードウェア選択を支える、ランナーの形状・料金・サイジングルール
- [リリースラウンド: develop→main ブランチ戦略](../real-world-patterns/release-rounds-branch-strategy.mdx) -- T2/T3の重いレーンをすべてのPRではなくベースブランチでゲートする
