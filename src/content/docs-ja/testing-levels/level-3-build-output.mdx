...
`--project build`は、Vitestの設定で名前付きの`build`プロジェクトを定義していれば、それにスコープした実行になります（[Vitestパターン](../real-world-patterns/vitest-patterns.mdx)を参照）。名前付きプロジェクトがない場合は、ビルド後に素の`vitest run`を実行しても同じように機能します。
