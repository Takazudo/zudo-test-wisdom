...
Every project gets T0 and T1, no justification needed. T0 is feedback speed -- what the agent runs constantly while implementing. T1 is **the authoritative gate**: a PR is mergeable when its required checks are green, and nothing outside T1 blocks a merge. Everything above T1 exists to serve it, not to replace it.
