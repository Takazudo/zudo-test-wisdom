...
### Mechanism 1: scope build and `webServer` to one fixture via env var

Introduce a single `FIXTURE=<name>` env var that narrows **both** the build step and the `webServer` array to just the target fixture. The inner loop then builds one site and boots one server -- no N-way stagger, no port-race. CI (no env var set) still builds and serves all N.
...
const allServers = [
  { command: "pnpm build:site-a && pnpm preview:site-a", url: "http://localhost:4001", name: "site-a" },
  { command: "pnpm build:site-b && pnpm preview:site-b", url: "http://localhost:4002", name: "site-b" },
  { command: "pnpm build:site-c && pnpm preview:site-c", url: "http://localhost:4003", name: "site-c" },
];

export default defineConfig({
  webServer: target ? allServers.filter((s) => s.name === target) : allServers,
  // ...
});
```

When developing: `FIXTURE=site-a pnpm test` — one build, one server, feedback in seconds.
