...
- **A merge that folds 12 tests into 1** recovers at most 11 tests' worth of unit cost — about 108 worker-seconds when the merged tests shared their entire setup (a cluster-merge), and less when per-iteration work such as navigation survives the merge (a loop-collapse), because what a merge removes is the per-test overhead, not the assertions' own work. If the merge takes an hour to do safely and the suite is nowhere near its budget, the price says do not bother.
...
- name: Record suite budget
  ...
  run: |
    ...
    secs=$(jq '.stats.duration / 1000 | round' results.json)
    {
      echo "| metric | value |"
      echo "|---|---|"
      echo "| e2e tests | ${tests} |"
      echo "| suite seconds | ${secs} |"
    } >> "$GITHUB_STEP_SUMMARY"
```

The duration comes from the reporter's own `stats.duration` (milliseconds), not from the shell: `$SECONDS` restarts at zero in every `run:` block, so in this recording step it would time the `jq` calls — a permanently near-zero number — rather than the e2e step it is meant to track. The point of putting it in the step summary rather than a log line is that drift is a trend: two numbers visible on every PR page make a slow climb legible without anyone running an analysis.
