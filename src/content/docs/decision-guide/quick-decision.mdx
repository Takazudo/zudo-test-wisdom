...
| Broken link / missing page / stale sitemap | [Site-integrity gates](../real-world-patterns/site-integrity-checks.mdx) (L3/L4 hybrid) | Structurally invisible to unit and component tests -- only a built-output/crawl check sees it |
| Deploy config: domain, route, binding id, certificate | [Post-deploy smoke test](../real-world-patterns/post-deploy-smoke-tests.mdx) | The failure is not in the repository, so nothing that runs against the repository can see it -- only a request to the deployed origin can |
| Visual regression risk across a refactor | [Committed-baseline visual regression](../real-world-patterns/visual-regression-baselines.mdx) | Deterministic pixel-diff gate -- catches shifted/recolored visuals an informal screenshot look would miss |
