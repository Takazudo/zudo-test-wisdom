...
| Broken link / missing page / dead image / stale sitemap | [Site-integrity gates](../project-recipes/site-integrity-checks.mdx) (L3/L4 hybrid) | Structurally invisible to unit and component tests -- only a built-output/crawl check sees it |
...
| A merge landed with nothing red, but no workflow ran at all | [CI-Skip Markers and Merge Hygiene](../ci-operations/skip-markers-and-merge-hygiene.mdx) |
| A check that only ever runs at publish time (`prepack`, `prepublishOnly`) | [Publishing-Pipeline Verification § Publish-Only Gates Are a Lane No Tier Runs](../ci-operations/publishing-pipeline.mdx#publish-only-gates-are-a-lane-no-tier-runs) |
| The PR gate is slow, but no single test is heavy | [Aggregate Suite Budget](./aggregate-suite-budget.mdx) |
...
- [Aggregate Suite Budget](./aggregate-suite-budget.mdx) — the suite-level half: when every test passes the per-test rules and the gate still drowns, price tests by unit cost and let budget drift trigger a prune.
