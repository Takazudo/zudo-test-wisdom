...
- **It still runs somewhere, allowed-to-fail.** `cargo test -- --ignored` runs only the quarantined set, and `cargo test -- --include-ignored` runs everything; schedule one of these as a T3 allowed-to-fail job so fresh failure data keeps flowing into the tracking issue.
</Note>
