...
The breakage appears *only after hydration runs in a real browser*, so nothing that inspects static output can see it. This is a **structural, deterministic** bug -- not a [hydration race](../test-integrity/deflaking-recipe.mdx) (a flake). It also recurs: one project hit this exact class twice, in two different island components.
