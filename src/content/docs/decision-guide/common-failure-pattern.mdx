...
|-------------|--------|-----------------|-----------------|
| `display: none` | Element removed from flow | Partial* | Yes |
...
\* Exception: jest-dom's `toBeVisible()` does catch `display: none` set inline (`style="display: none"`) or via a `hidden` attribute — jsdom resolves these directly, and its CSSOM can even match simple `<style>`-tag selectors when that stylesheet is actually present in the test's DOM. What defeats Level 2 in practice is usually scope: a component test renders the component alone, so an ancestor stylesheet like `.main-content` above is never loaded into that DOM at all. And for geometry-driven hiding — clipping, off-screen positioning, stacking — jsdom has no layout engine, so Level 2 cannot see it regardless of how the CSS was authored.
