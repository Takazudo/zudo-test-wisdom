...
```sh
# One-time setup: add the gate tools as dev dependencies
pnpm add -D html-validate linkinator
...
- Ships **platform-specific binaries or an exec bit** → add publish-time pack verification; the three failure modes a plain unit suite cannot see are in [Publishing-Pipeline Verification](../ci-operations/publishing-pipeline.mdx).
- **Consumed by pin across other repos** → add a scheduled registry drift net (**T3**) so a breaking publish is caught before consumers hit it — see [Scheduled Re-exam](../ci-operations/scheduled-re-exam.mdx).
...
Mock the binding, not the runtime — the setup patterns are in [Backend & Node.js Testing](../project-recipes/backend-testing.mdx#mock-the-binding-not-the-runtime).
...
The core-crate and backend-bridge-mock patterns are in [Tauri App Testing](../project-recipes/tauri-testing.mdx#backend-bridge-mock-adapter-pattern).
...
Guard anything that mutates prod behind an explicit destructive-test guard — see the environment-based URL switching and destructive-test-guard patterns in [HTTP API Testing](../project-recipes/backend-testing.mdx#http-api-testing).
