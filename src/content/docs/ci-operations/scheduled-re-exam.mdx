...
description: Operating heavy, platform-bound test lanes on a schedule -- the CI re-exam workflow, deduped failure issues, on-demand pre-merge dispatch, and the local night exam with agent triage.
sidebar_position: 3
---
...
- **Runner shapes, pricing, and sizing rules live elsewhere.** See [CI Runner Sizing](./ci-runner-sizing.mdx) for the label matrix, macOS cost multipliers, and the four rules for when a bigger or different runner actually helps.
...
Synthetic fixtures are safe only when **you own the contract**. For a third-party tool's output you cannot author truth -- you can only capture it. Hand-authored fixtures are the right choice when the reader owns the schema being tested: for example, mdast tree factories in [remark/rehype plugin tests](../project-recipes/remark-rehype-plugin-testing.mdx) (you own the mdast contract) or synthetic inner-bundle objects in [level-3 build-output tests](../testing-levels/level-3-build-output.mdx) (you own the bundle shape). For a tool like Playwright, Jest, or Vite whose output schema is theirs to change, capturing a real artifact is the only way to stay honest.

</Warning>

The same rule has a second instance one tool over: nextest's `junit.xml` nests every retried attempt as a `<flakyFailure>`, `<flakyError>`, `<rerunFailure>`, or `<rerunError>` child, and [pricing retries from that real shape](../tool-patterns/cargo-nextest-migration.mdx#after-migration-optimize-without-losing-evidence-price-retries-from-the-real-nextest-junit-shape) needs a captured report -- a healthy suite emits none of those elements, so a hand-authored fixture is the only thing a naive parser will ever have been tested against.
...
`if-no-files-found: error` here fires only when **both** paths are empty. It does not catch the miss this warning is about, where `playwright-report/` exists and `test-results/` does not — one matching path is enough for the step to upload and stay green.

The placement cuts the other way too: `outputDir` (`test-results/`) is **emptied at the start of every run**, so anything you park there to keep across runs is deleted by the next one — see [Artifacts You Want to Keep Must Live Outside `outputDir`](../tool-patterns/playwright-patterns.mdx#artifacts-you-want-to-keep-must-live-outside-outputdir).
