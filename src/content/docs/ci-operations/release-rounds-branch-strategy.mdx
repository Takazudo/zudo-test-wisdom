...
The pattern here was evaluated 2026-07-05 for adoption in pgen (`zudolab/zudo-pattern-gen`), Takazudo's private GPU pattern generator with a Cloudflare Workers backend, and is already proven in another project's release flow. It is written for the specific shape that makes it pay off: a high-volume, agent-heavy workflow shipping into a `main` branch that auto-deploys.
...
- **Per-PR heavy CI cost multiplies by PR volume.** Any heavy lane you attach to every PR -- long end-to-end suites, [visual regression](../tool-patterns/visual-regression-baselines.mdx), Mac or GPU runners -- runs 200+ times a month whether or not the change could possibly have touched what it covers. The heavy lane, not the feature work, becomes the dominant line item on the CI bill.
...
**Never let the skip marker reach a base-`main` PR.** The `[skip test]` opt-out exists so an author can iterate quickly on a `develop`-targeting branch without waiting on the full gate every push. If that same marker could suppress the gate on a round PR, one stray commit message would ship untested code straight to the production deploy. The `base_ref == 'main'` check has to be evaluated *first* and unconditionally, exactly as above -- the invariant is that no marker, label, or convention can bypass the gate on the branch that changes production. Note also what this check *cannot* see: a GitHub-native `[skip ci]` suppresses the workflow itself, so this step never exists to evaluate it -- no in-workflow guard can catch that hazard, and only a required status check (which leaves the suppressed run pending) closes it -- the mechanics, and the merge-message guard that covers the squash side, are in [CI-Skip Markers and Merge Hygiene](./skip-markers-and-merge-hygiene.mdx).
...
- [CI-Skip Markers and Merge Hygiene](./skip-markers-and-merge-hygiene.mdx) -- the second, independent reason not to squash: a squash merge can copy a CI-skip marker out of branch history and onto `main`, suppressing every `push` workflow for that commit.
