...
Most of this guide tests code before it ships: unit suites, build-output reads, a browser driven against a local dev server. A separate layer tests the thing that actually shipped -- the **same suite pointed at a deployed URL**. Two portfolio projects run this as a first-class lane: [Takazudo Modular](https://takazudomodular.com/), Takazudo's own hardware webshop (source not public), fires its preorder-API contract suite at a local dev server, at a preview deploy, and at production; takazudo-auth, a session-gated auth app, drives logged-in flows against the same tiers. This page is about running one suite across many environments without forking your test command three ways -- and without ever mutating production.
...
Preview deploys are usually behind a password wall (Netlify password protection, Cloudflare Access). The whole deploy sits behind one gate cookie, so an automated run has to unlock it before it can see a single route. Trade the gate password -- a CI secret, never a literal -- for that cookie once, then replay the cookie on every request. Takazudo Modular documents this specifically so **AI-agent runs** can reach a protected preview the same way CI does.
...
Passing the deploy gate only gets you to the login screen. Flows that need an authenticated *user* -- takazudo-auth's whole surface sits behind a login -- log in once, persist the session with Playwright's `storageState`, and reuse it across the run. The credentials come from the environment; they never appear in a spec.
...
- [Backend & Node.js Testing](../project-recipes/backend-testing.mdx) -- the HTTP-API base-URL switch, destructive-test guards, and the post-deploy smoke script this page builds on
- [Execution Tiers](../decision-guide/execution-tiers.mdx) -- the T0-T4 vocabulary that decides where each environment lane runs
- [Playwright Patterns](../tool-patterns/playwright-patterns.mdx) -- CI-safe vs environment-heavy test splitting, the mock-backend adapter, and console-error monitoring for these live runs
...
- [Fail-Closed Live Proofs and Evidence Contracts](../test-integrity/live-proof-evidence-contracts.mdx) -- the one fenced exception to the preview tier's read-only row: a release proof whose operation *is* a write, run on a disposable generation, and the three verdicts it owes in return
