...
[Level 3 build-output tests](../testing-levels/level-3-build-output.mdx) read `dist/` on disk, and [site-integrity gates](./site-integrity-checks.mdx) crawl a *locally served* build. Both stop at the artifact. A post-deploy smoke test starts where they end:
