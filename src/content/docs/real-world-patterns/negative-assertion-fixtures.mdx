...
The tell that separates this from a genuine regression: **every positive assertion in the same test passes, and only the absence assertion fails.** A real resolver regression would also break the positive checks that depend on correct resolution. When the pipeline demonstrably worked -- the payload is present, the import resolved, the build succeeded -- but a lone absence assertion is red, suspect the fixture before the product, and run the fixture-tree grep from **Rule 1** first.
