...
- **Drop the line number, never the file.** The line is what drifts when spec files shift between commits; the file is what disambiguates. Degrading from `project + file + line + title` to `project + file + title` stays correct — degrading to `title` does not.
