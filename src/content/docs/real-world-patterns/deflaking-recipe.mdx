...
- Inputs **differ** → a **product bug**. Stop. Fix the product. See [The impostor](#the-impostor-when-the-flake-is-a-real-product-bug).
...
- **Platform-varied input shape** — the odd one out, and the only cause on this list where **the test is not the problem at all**. See [below](#the-impostor-when-the-flake-is-a-real-product-bug).
...
grep -E "  ✓ " run.log |
  sed -E 's/.*› ([^ ]+\.spec\.ts:[0-9]+:[0-9]+) › .*\(([0-9.]+m?s?)\)$/\1\t\2/' |
  grep -v retry > run.times
