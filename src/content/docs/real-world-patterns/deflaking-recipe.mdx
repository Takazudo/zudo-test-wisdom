...
Rebuilding the same diff **locally** from two JSON reports instead? Both reports must be written outside Playwright's `outputDir` — the start-of-run wipe deletes run 1's report before run 2 begins, and the comparison becomes structurally impossible. See [Artifacts You Want to Keep Must Live Outside `outputDir`](./playwright-patterns.mdx#artifacts-you-want-to-keep-must-live-outside-outputdir).
...
- **It reproduces on `retries: 0` locally and never in CI,** where `retries: 2` absorbs it — see [the retries-asymmetry trap](./playwright-patterns.mdx#quarantining-flakes-the-retries-asymmetry-trap).

<Warning>

**Do not quarantine the spec that happened to lose the race.** It is an existing test on `main` that genuinely went red on the zero-retry gate, so it clears the "never tag a brand-new test" guard on [`@flaky`](./playwright-patterns.mdx#quarantining-flakes-the-retries-asymmetry-trap) — and tagging it is still wrong. You would suspend coverage of a spec that has no defect, leave the lane defect fully intact, and hand it a fresh victim next run. Repeat that three runs running and you have quarantined three healthy specs and fixed nothing.
...
Reports must also be written **outside** Playwright's `outputDir`: it is emptied at the start of every run, so run 2 deletes run 1's report and the comparison is structurally impossible. Point `PLAYWRIGHT_JSON_OUTPUT_NAME` at a sibling directory (`b4push-reports/`), not `test-results/` — see [Artifacts You Want to Keep Must Live Outside `outputDir`](./playwright-patterns.mdx#artifacts-you-want-to-keep-must-live-outside-outputdir).

</Warning>

Once confirmed, the lever is the lane's own budget — worker count first, then trace retention and per-test timeout. This is the Playwright analogue of [capping vitest parallelism in the heavy local gate](./vitest-patterns.mdx#project-level-vitest-configs-cap-worker-parallelism-in-the-heavy-local-gate), and for the same reason: a heavy local gate buys reliability with wall-clock, because a gate that reds out on a healthy spec is worth less than a gate that takes two more minutes.
...
|---|---|---|
| Specs time out on a 2-vCPU CI runner | Cores | 2→4 vCPU ([runner-sizing Rule 3](../tools-reference/ci-runner-sizing.mdx#the-four-runner-sizing-rules-rule-3-diagnose-the-failure-mode-before-resizing)) |
...
**The same key governs issue filing.** A tracking issue titled `flaky: closes with Escape` is one issue collecting telemetry from two unrelated specs, and neither spec's fix will ever close it cleanly. Title issues with the identity — `flaky: e2e/settings-dialog.spec.ts › closes with Escape` — so the dedup query matches one spec and only that spec (and when the same spec runs under several Playwright projects, put the project in the title too). This is the same discipline as the workflow-name-in-the-title rule for [deduped exam issues](./scheduled-re-exam.mdx#one-tracking-issue-never-one-per-run): the dedup key has to be as specific as the thing it identifies.

</Note>

### The twin-leg comparison: same commit, same suite, one leg red

Every comparison above joins runs across **time** — this run against an earlier run, durations drifting, identities repeating. There is a sharper comparison available whenever the pipeline runs the same suite more than once on the same commit: a required check plus an identical sibling matrix leg, a re-triggered workflow, two workflows that both run the suite. One leg passes everything; the other fails one test. Same tree, same suite, same moment — and it costs zero extra runs, because the pipeline already paid for both.

What it proves is immediate and cheap: **the failure is intermittent on this commit.** One execution of this exact tree produced green and another produced red, so the hypothesis "the diff deterministically broke this test" is dead on arrival — no reproduction run needed, no bisect. For a [gate operator](#red-ci-you-don-t-own-—-the-gate-operator-s-playbook) mid-incident, that is the fastest possible narrowing of the question.

Two limits keep it honest. First, **twin legs classify intermittency, not causality.** The pair does not clear the diff: an intermittent race the diff just *introduced* produces exactly the same green/red split, and the legs are not a laboratory control group anyway — runner image, cache state, external services, and timing all differ between them. Exoneration needs history on top: the same identity failing before the diff existed, or concurrently on the base branch, plus a transitive diff-scope check (shared fixtures, config, dependencies, global setup). Second, the pair says nothing about the failing identity's behavior *across* runs — whether it roams (a lane property — [entry 7](#part-1-—-root-cause-catalog-7-lane-over-subscription-the-failure-whose-identity-roams), including its hosted-CI variant) or repeats (a per-spec fix candidate, or [the impostor](#part-1-—-root-cause-catalog-the-impostor-when-the-flake-is-a-real-product-bug)) still requires the identity-keyed cross-run history above.
