...
`pnpm pack` normalizes file modes to `0644`, silently stripping the `0755` executable bit off a shipped binary. The install succeeds, the file lands on disk, and spawning it fails with `EACCES` -- another green pipeline, another runtime-only failure. There are two gates here, and they work together.
