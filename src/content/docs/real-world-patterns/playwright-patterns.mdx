...
The production-build advice above has a monorepo-specific blind spot. Many app suites *cannot* run against the production build: MSW's service-worker setup, dev-only routes, and dev-only test bridges (state exposed on `window` behind an `import.meta.env.DEV` guard) all exist only under the dev server. But running the dev server in a pnpm-workspace monorepo has a hidden per-test tax.
...
- **The `existsSync` guard throws.** Without it, a missing or stale `dist` silently falls through to source-serving (or worse, tests a stale bundle) and the lane's numbers become meaningless.
...
- **Only the heavy, stable package is bundled.** The app's own source — the code under test — is still served live. You are bundling a dependency, not the subject.
