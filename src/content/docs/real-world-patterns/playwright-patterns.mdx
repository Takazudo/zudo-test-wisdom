...
- **The `existsSync` guard throws on a missing bundle.** Note what it cannot do: detect a *stale* one — freshness comes from the CI step building the package immediately before Playwright, not from the guard. Without the guard, a missing `dist` would silently fall through to source-serving and the lane's numbers become meaningless.
