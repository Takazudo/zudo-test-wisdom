...
The fix mirrors the single-source-of-truth meta-test pattern: a tiny script that asserts every `e2e/*.spec.ts` filename starts with a known project prefix, wired into both the local pre-push gate and CI:
...
<Tip>
`waitForLoadState("networkidle")` is the canonical anti-pattern for SPA navigations that fire no network requests — it can resolve before the view is ready. Use a web-first assertion (like `expect(...).toBeVisible()`) or an event-keyed wait as the real completion signal. See [Flake Root-Cause Catalog & Deflaking Recipe](./deflaking-recipe.mdx) for the full catalog and recipe.
</Tip>
