...
flowchart TD
  ...
  B -->|Waits time out while<br/>CPU sits idle| E[Connection starvation<br/>see Note below]
...
A third failure mode presents identically and answers to **neither** lever: **connection starvation** — the browser's per-host connection pool (6 on HTTP/1.1) exhausted by a dev-server module storm, so waits time out while CPU sits idle. The pool does not grow with vCPU; the fix is at the app/serving layer. See [catalog entry 6 in the Flake Root-Cause Catalog](../real-world-patterns/deflaking-recipe.mdx#part-1-—-root-cause-catalog-6-in-app-background-work-starving-unrelated-waits-connection-starvation).
