...
The cancellation breaks in two ways, and only one of them looks serial. Work that is *inherently* serial — a single-threaded build step, a serial migration — gets **no** wall-clock benefit from more vCPU. But work that is inherently parallel can be **artificially capped**: an explicit `workers: N` in `playwright.config.ts` (or a `--workers` CLI flag) pins concurrency, so the "workers scale with cores" leg of the cancellation disappears — cost still scales linearly while wall-clock does not move. "It's Playwright" is not proof the job parallelizes; check the config for a pinned `workers` value before sizing up. In both cases a bigger runner is pure added cost.
