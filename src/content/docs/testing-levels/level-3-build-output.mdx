...
```sh
pnpm build && vitest run --project build
```

`--project build` scopes the run to a named `build` project if your Vitest config defines one (see [Vitest Patterns](../real-world-patterns/vitest-patterns.mdx)); without a named project, a plain `vitest run` after the build works the same way.
