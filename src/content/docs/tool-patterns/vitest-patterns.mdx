...
Telling them apart matters because the standard deflaking prescription -- "replace the wall-clock wait with an event-keyed one" -- is a no-op against the second kind: there is no wall-clock wait left to replace, only a number to justify. What both senses do share is one discipline: raise either only from a measured latency distribution, never on a hunch. The 60s project budget above qualifies because it is derived from the largest child timeout it has to contain; a number picked to turn a red run green does not remove a flake, it hides one.
