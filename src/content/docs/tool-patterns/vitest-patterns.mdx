...
# --reporter=json keeps per-file durations and counts so parity can be
# diffed; /usr/bin/time -l (macOS) or -v (GNU) reports the machine-wide
# peak RSS that a per-process heap number cannot see.
COMMON="run --project unit --maxWorkers=4 --reporter=json"

/usr/bin/time -l pnpm vitest $COMMON --outputFile=baseline.json
/usr/bin/time -l pnpm vitest $COMMON --outputFile=candidate.json --pool=threads
...
pnpm vitest $COMMON --outputFile=shuffled.json --no-isolate --sequence.shuffle.files
```
