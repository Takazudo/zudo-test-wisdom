...
description: Two complementary layers for AST plugins -- hand-built mdast tree factories for unit control, and a golden-fixture corpus, with UPDATE_FIXTURES regeneration and a cross-language parity oracle, for whole-pipeline parity.
sidebar_position: 3
---
...
The byte-for-byte bar is specific to a **faithful port** -- the same pipeline reimplemented in another language, where the reference defines truth by construction and any difference is therefore a porting defect. It does not carry over to swapping one dependency for another. A replacement library ships its own data snapshot and its own deliberately fixed bugs, so a small *classified* delta is frequently the correct outcome there, and insisting on byte identity would mean rejecting the candidate for being more correct. [Dependency Replacement Testing](./dependency-replacement-testing.mdx) keeps this pattern's discipline -- a pinned reference and a mechanical diff -- with the acceptance criterion that case needs: zero *unclassified* differences rather than zero differences.
