...
For content-heavy sites the dominant failure mode is not a component bug at all -- it is **data integrity**: a broken link, a dead image, a stale canonical, a missing page. The audit lane of [Takazudo Modular](https://takazudomodular.com/) -- Takazudo's own hardware webshop and product-catalog site, source not public -- confirmed this directly: its canonical/sitemap/link/OGP checks and its 404 sweep catch far more real regressions than its component tests do. The gates below exist because the site's actual failures live in the built output, not in any single unit of code.
...
This pairing -- a strict link check plus an html-validate pass -- is a first-class CI gate on multiple sibling projects (a component-library and style-guide monorepo, [`zudosg`](https://github.com/Takazudo/zudo-sg), runs both on every PR), and [`zudo-doc`](https://github.com/zudolab/zudo-doc), the doc-site framework this guide is built with, classifies link-check + html-validate + preview-smoke together as its **Level 3 build-output gate**.
...
- **A Playwright crawl-all-pages spec** navigates every route in a real browser and asserts on rendered structure -- catches hydration errors and client-side failures a fetch cannot see. [`zmanuals`](https://github.com/Takazudo/takazudomodular-manuals), Takazudo Modular's hardware-manual site, keeps exactly this as its crawl-all-pages spec.
- **A fast smoke crawler script** issues a plain HTTP request to every URL (from the sitemap or by following links) and asserts the status code -- no browser, so it runs in seconds instead of minutes. `zmanuals` keeps this *alongside* the Playwright spec as the fast lane; Takazudo Modular runs the same shape as `all-articles`, `all-brands`, and `all-categories` crawls, one per content collection.
...
Link check, html-validate, and crawl smoke are the well-known members. The same tier holds a wider family of deterministic sweeps over the built output, and on content-heavy sites these catch the most. Takazudo Modular runs the first four below as a standing set:

- **404 sweep.** Assert that every URL that *should* exist does, and optionally that known-bad URLs correctly 404. There, it runs as a dedicated scheduled lane.
- **OGP sweep.** Every page must carry a complete, non-empty Open Graph block -- `og:title`, `og:description`, `og:image` -- because a missing tag only surfaces when the page is shared, far from any test. The sweep runs as a built-output gate.
- **Image-health sweep.** Every `<img src>` in the built HTML must resolve to a file that exists and is non-empty. A standalone image-health script covers exactly that.
- **Canonical and sitemap audits.** Cross-check the sitemap against the pages actually emitted, and every canonical tag against a page that exists. These share a single pass with the link and OGP checks.
