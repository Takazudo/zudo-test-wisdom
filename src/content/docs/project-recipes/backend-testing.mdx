...
Miniflare emulates the Workers runtime in Node. `@cloudflare/vitest-pool-workers` goes one step further: Vitest test files run **inside** an actual `workerd` isolate, with real D1/KV/R2 bindings wired through `wrangler.toml` rather than a Node-side emulation layer.
