...
description: Testing patterns for server-side Node.js code with Vitest -- Cloudflare Functions on Miniflare, HTTP API testing, fetch mocking with vi.stubGlobal, temp-directory file system testing, and the principles for separating frontend and backend test configs.
sidebar_position: 1
---
...
## Real workerd Testing with vitest-pool-workers

Miniflare emulates the Workers runtime in Node. `@cloudflare/vitest-pool-workers` goes one step further: Vitest test files run **inside** an actual `workerd` isolate, with real D1/KV/R2 bindings wired through `wrangler.toml` rather than a Node-side emulation layer. Live practice: zudo-file-sync -- a private file-sync service on Cloudflare Workers -- runs its in-worker suites this way.
...
**The decision rule, extended to a third option.** `Map`-stub: fast, no platform semantics, for handler logic (above). Miniflare: binding *behavior* emulated in Node, for SQL/storage semantics (above). `@cloudflare/vitest-pool-workers`: reach for this only when the platform semantics themselves are under test -- D1 quirks that only reproduce in real workerd, `waitUntil` timing, bindings wiring, or an in-worker end-to-end path. It is the slowest and heaviest of the three; use it when Miniflare's own emulation is the thing in question, not your handler.
